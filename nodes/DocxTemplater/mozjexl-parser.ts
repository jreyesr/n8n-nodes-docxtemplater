/**
 * Parser for Docxtemplater that uses the Mozilla project `mozjexl` for expression parsing.
 * Similar in spirit to the built-in Angular parser, but this parser supports async expressions for filters,
 * which aligns better with N8N's "AI Tool" invocation semantics
 *
 * @see https://docxtemplater.com/docs/
 * @see https://docxtemplater.com/docs/deep-dive-into-the-parser-option/#viewport
 * @see https://github.com/mozilla/mozjexl
 */
...
// function validStartChars(ch: string) {
// 	return startRegex.test(ch);
// }
//
// function validContinuationChars(ch: string) {
// 	return middleRegex.test(ch);
// }

function getIndex(scope: any, context: any) {
	let index = null;
...
// function getObjectIdentifiers(x: any, scope: any = {}) {
// 	if (x.expression) {
...
// function getIdentifiers(x: any): any {
// 	if (x.expression) {
// 		return getIdentifiers(x.expression);
// 	}
// 	if (x.body) {
// 		return x.body.reduce((result: any[], y: any) => result.concat(getIdentifiers(y)), []);
// 	}
// 	if (x.type === 'CallExpression') {
// 		if (x.arguments) {
// 			return x.arguments.reduce((result: any[], y: any) => result.concat(getIdentifiers(y)), []);
// 		}
// 	}
// 	if (x.ast) {
